...
bedtools merge -i Ler0-vs-ALL.sort.bed > merged_windows_Ler0-vs-ALL.sort.bed 
...
mkdir results/
cd results/
cp ../merged_windows_Ler0-vs-ALL.sort.bed .
...
awk 'BEGIN{ FS = OFS = "\t" } { print $0,"TIP_finder_"NR"_CM"}' merged_windows_Ler0-vs-ALL.sort.bed > named.merged_windows_Ler0-vs-ALL.sort.bed
